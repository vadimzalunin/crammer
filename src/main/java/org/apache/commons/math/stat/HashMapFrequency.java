 ...
 * comparable to those that have been added, otherwise the add method will throw
 * an IllegalArgumentException.
 * </p>
 * <p>
 * Integer values (int, long, Integer, Long) are not distinguished by type --
 * i.e. <code>addValue(Long.valueOf(2)), addValue(2), addValue(2l)</code> all
 * have the same effect (similarly for arguments to <code>getCount,</code>
 * etc.).
 * </p>
 * <p>
 * char values are converted by <code>addValue</code> to Character instances. As
 * such, these values are not comparable to integral values, so attempts to
 * combine integral types with chars in a frequency distribution will fail.
 * </p>
 * <p>
 ...
 * </p>
 * 
 * @version $Revision: 1054186 $ $Date: 2011-01-01 03:28:46 +0100 (sam. 01 janv.
 *          2011) $
 */
public class HashMapFrequency implements Serializable {

	/** Serializable version identifier */
	private static final long serialVersionUID = -3845586908418844111L;

	/** underlying collection */
	private final HashMap<Comparable<?>, Long> freqTable;

	/**
	 * Default constructor.
	 */
	public HashMapFrequency() {
		freqTable = new HashMap<Comparable<?>, Long>();
	}
	...
	public String toString() {
		...
		Comparable<?>[] array = new Comparable<?>[freqTable.size()];
		int i = 0;
		while (iter.hasNext()) {
			array[i++] = iter.next();
		}
		...
		for (i = 0; i < array.length; i++) {
			...
			double pct = sumFreq == 0 ? Double.NaN : (double) count / (double) sumFreq;
			cumPct += pct;
			...
			outBuffer.append(nf.format(pct));
			outBuffer.append('\t');
			outBuffer.append(nf.format(cumPct));
			outBuffer.append('\n');
	 ...
	 * <p>
	 * If other objects have already been added to this Frequency, v must be
	...
	public void addValue(Object v) {
		if (v instanceof Comparable<?>) {
			...
			throw MathRuntimeException.createIllegalArgumentException(
					LocalizedFormats.CLASS_DOESNT_IMPLEMENT_COMPARABLE, v.getClass().getName());
		}
	}
	...
	/**
	 * Returns the number of values = v. Returns 0 if the value is not
	 * comparable.
	 * 
	 * @param v
	 *            the value to lookup.
	 ...
	 * 
	 * @return the number of unique values that have been added to the frequency
	 *         table.
	 * @see #valuesIterator()
	 */
	 ...
	 * <p>
	 * Returns <code>Double.NaN</code> if no values have been added. Returns 0
	 * if at least one value has been added, but v is not comparable to the
	 * values set.
	 * </p>
	 * 
	 * @param v
	 ...
	 * Copied from Commons Collections ComparableComparator.
	 */
	private static class NaturalComparator<T extends Comparable<T>> implements Comparator<Comparable<T>>, Serializable {
		...
		/**
		 * Compare the two {@link Comparable Comparable} arguments. This method
		 * is equivalent to:
		 * 
		 * <pre>
		 ...
		 *             when <i>o1</i> is <code>null</code>, or when
	...
	public boolean equals(Object obj) {
		if (this == obj)
			return true;
		if (!(obj instanceof HashMapFrequency))
			return false;
		HashMapFrequency other = (HashMapFrequency) obj;
		if (freqTable == null) {
