...
public class ArithCodec implements BitCodec<byte[]> {
    ...
    public int decodeCharacter(BitInputStream bis) throws IOException {
            ...
            while (true) {
                    ...
                    for (int i = 0; i < probs.length; i++) {
                            if (probs[i]>min) {
                                    if (probs[i]>max) val = i;
                                    break;
                            }
                    }
                    if (val==-1) {
                            boolean bit = false;
                            if ((fileData.get(curByte)&(128>>curBit)) != 0) bit = true;
                            if (bit) min = cur;
                            else max = cur;
                            curBit++;
                            if (curBit == 8) {
            ...
            while (true) {
                    double cur = (min+max)/2.0;
                    int temp = 0;
                    for (; temp < probs.length; temp++) if (probs[temp]>cur) break;
                    if (cur < 0 || cur > 1) temp = -1;
                    if (temp != val) {
                            boolean bit = false;
                            if ((fileData.get(curByte)&(128>>curBit)) != 0) bit = true;
                            if (bit) min = cur;
    ...
    private void encodeCharacter (BitOutputStream bos, int character) 
                    throws Exception {
            if (probs.length<2 || probs[probs.length-1]!=1 
                            || character < 0 || character>=probs.length)
                    throw new Exception("Invalid input");
            if (character > 0) localMin = probs[character-1];
            else localMin = 0;
    ...
    private void flush(BitOutputStream bos) throws IOException {
            if (curBit != 0) {
                    while (true) {
                            ...
                            double cur = (min+max)/2.0;
                            if (cur >= localMin && cur < localMax) break;
                    }
            }
            bos.close();
    }
