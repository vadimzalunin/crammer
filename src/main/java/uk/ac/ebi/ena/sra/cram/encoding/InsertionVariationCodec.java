...
public class InsertionVariationCodec implements BitCodec<InsertionVariation> {
	...
	@Override
	public long write(BitOutputStream bos, InsertionVariation v) throws IOException {
		long len = 0L;

		len += insertBasesCodec.write(bos, v.getSequence());
		// System.out.println(new String(v.getSequence()) + " encoded in " + len
		// + " bits.");
	...
	public String toString() {
		return String.format("Insertion codec: %d bases total, %d bits, %.2f bits per base.", byteCounter, totalLen,
				(float) totalLen / byteCounter);
	}
