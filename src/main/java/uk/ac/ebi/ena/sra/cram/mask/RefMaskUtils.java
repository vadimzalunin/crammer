...
public class RefMaskUtils {
	...
	public static void main(String[] args) {
		short mask = 0;
		for (byte i = 0; i < 16; i++) {
			mask = addReadBase(mask, (byte) 'A', (byte) 'A');
			System.out.println(Utils.toBitString(mask).substring(2 * 8));
		}

		for (byte i = 0; i < 16; i++) {
			mask = addReadBase(mask, (byte) 'C', (byte) 'A');
			System.out.println(Utils.toBitString(mask).substring(2 * 8));
		}

		for (byte i = 0; i < 16; i++) {
			mask = addReadBase(mask, (byte) 'G', (byte) 'A');
			System.out.println(Utils.toBitString(mask).substring(2 * 8));
		}

		for (byte i = 0; i < 16; i++) {
			mask = addReadBase(mask, (byte) 'T', (byte) 'A');
			System.out.println(Utils.toBitString(mask).substring(2 * 8));
		}
	}
