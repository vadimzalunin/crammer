...
public class DefaultReadFeaturesFormat implements ReadFeaturesFormat {
		...
		for (ReadFeature feature : features) {
			...
			if (feature.getPosition() > prevPos) {
				if (feature.getPosition() - prevPos == 1) {
					...
					sb.appendCodePoint(MATCH).append(feature.getPosition() - prevPos);
					qsRequired = false;
				}
			...
			case InsertionVariation.operator:
				...
				sb.appendCodePoint(INSERTION).append(new String(ins.getSequence())).append(".");
			...
			case BaseQualityScore.operator:
				...
				if (qsRequired)
					...
					sb.appendCodePoint('=').appendCodePoint(bqs.getQualityScore());
				qsRequired = false;
				break;
			case SubstitutionVariation.operator:
				...
				if (useOneLetterCodesForSubstitutions) {
					...
					sb.appendCodePoint(SUBSTITUTION).append(sub.getBaseChange().getChange());
			...
			default:
				throw new RuntimeException("Unkown read feature operator: '" + (char) feature.getOperator() + "'");
			}
	...
	public List<ReadFeature> asReadFeatureList(String string) {
		...
		for (int i = 0; i < bytes.length; i++) {
			...
			case MATCH:
				if (bytes.length <= i + 1)
					throw new IllegalArgumentException("A positive integer is expected after 'M' operator in " + string
							+ " at " + i + "th position.");
			...
			case DELETION:
				if (bytes.length <= i + 1)
					throw new IllegalArgumentException("A positive integer is expected after 'D' operator in " + string
							+ " at " + i + "th position.");
			...
			case SUBSTITUTION:
				if (bytes.length <= i + 1)
					throw new IllegalArgumentException("A positive integer is expected after '" + (char) SUBSTITUTION
							+ "' operator in " + string + " at " + i + "th position.");
			...
			case '=':
				...
				break;
			default:
				throw new IllegalArgumentException("Unkown operator: " + bytes[i] + " at " + i + " in: " + string);
			}
		}
		return features;
	}
	...
	private static final int readPositiveInt(byte[] bytes, int from) {
		...
		} while (value < maximumAllowedValue && ++pos < bytes.length && bytes[pos] >= '0' && bytes[pos] <= '9');
		return value;
	}

	private static final byte[] readNonEmptyUntil(byte[] bytes, int from, byte stopByte) {
		int pos = from;
