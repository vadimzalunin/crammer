...
public class CramExample {
	...
	public static void main(String[] args) throws IOException, CramCompressionException {
		...
		// prepare header here:
		header.setReferenceSequences(Arrays
				.asList(new CramReferenceSequence(refSeqName, referenceSequenceBases.length)));
		// ...
		CramHeaderIO.write(header, os);

		SequenceBaseProvider provider = new ByteArraySequenceBaseProvider(referenceSequenceBases);
		SequentialCramWriter writer = new SequentialCramWriter(os, provider, header);
		...
		for (int blockIndex = 0; blockIndex < nofBlocks; blockIndex++) {
			...
			for (int recordIndex = 0; recordIndex < recordsInBlock; recordIndex++) {
				...
				record.setReadFeatures(Arrays.asList(new ReadFeature[] { new InsertBase(1, (byte) 'A') }));
				// ...
